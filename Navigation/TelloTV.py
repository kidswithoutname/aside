...
# from utils import rect, circle, center, bbox2, rescale_center, rescale_box, np_multi_tofile
...
if args.save_session:
    ...
    # For depth
    ddird = "Sessions/depth/Session {}".format(stamp).replace(':', '-').replace('.', '_')
    os.mkdir(ddird)

    # For pred
    ddirp = "Sessions/pred/Session {}".format(stamp).replace(':', '-').replace('.', '_')
    os.mkdir(ddirp)
...
class FrontEnd(object):
    ...
    def run(self):
        ...
        #  ------------------------------- BEGIN Net Construction ADDED ------------------------------------------ #
        #  Contruct net
        input_node, net = contruct_net()
        #  ------------------------------- END Net Construction ADDED -------------------------------------------- #
